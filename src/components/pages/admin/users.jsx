...
import { deleteUser } from "../../../services/restUserService";
    ...
    <div className="container overflow-x-auto container-admin">
      ...
      <div className="table-container">
        <div className="d-flex align-items-center justify-content-between px-4 header-admin">
          <h2 className="fw-normal fs-1 heading-admin">Danh sách người dùng</h2>
        </div>
                ...
                <>
                  {kind.role === "1" && (
                    <tr key={kind.id}>
                      ...
                      <td>
                        <div className="infor">
                          <img src={kind.avt} alt={`${kind.username}_avt`} />
                          <span>{kind.username}</span>
                        </div>
                      </td>
                      <td>{kind.username}</td>
                      <td>
                        {new Date(kind.birthday).toLocaleDateString("en-GB")}
                      </td>
                      <td>
                        {kind.role === "1" ? (
                          <button
        ...
        <div className="d-flex pagination-admin">
          <div className="col-6 description-pagination">
            <div style={{ fontSize: "medium" }}>
              ...
              trong <span style={{ color: "red" }}>{maxpage}</span> người dùng
            </div>
          </div>
