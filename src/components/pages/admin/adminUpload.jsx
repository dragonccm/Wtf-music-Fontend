...
const AdminUpload = () => {
  ...
  const createMusicSongs = async () => {
    try {
      ...
      const newData = {
        ...data,
        artists: data.artist.map((data) => data.id).join(","),
        genresid: data.genre.map((data) => data.genreId).join(","),
      };
  ...
  const handleDelete = (chipToDelete, type) => () => {
    if (type === "genre") {
      setData((data) => {
        const newData = { ...data };
        newData.genre = newData.genre.filter(
          (chip) => chip.genreId !== chipToDelete.genreId
        );
        return newData;
      });
    } else {
      setData((data) => {
        const newData = { ...data };
        newData.artist = newData.artist.filter(
  ...
  const handleAdd = (chipToAdd, type) => () => {
    if (type === "genre") {
      setData((data) => {
        const newData = { ...data };
        const exists = newData.genre.some(
          (chip) => chip.genreId === chipToAdd.genreId
        );
        if (!exists) {
  ...
  const handleAudioUpload = (audioFile) => {
    ...
    console.log("Audio file uploaded:", URL.createObjectURL(audioFile));
  };
  ...
  return (
    <div className="edit">
      <h1 className="edit_title">Upload Nhạc</h1>
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        name="data"
                        onChange={(event) =>
                          handleInputChange(event, word._id, "data", lineIndex)
                        }
                      />
                      :
                      <div className="change_time_ctn">
        ...
        <div className="edit_body_info">
          <div className="edit_body_info_head">
            <div className="edit_body_info_img">
              {imageUrl ? (
                <img src={imageUrl} className="song-img" alt="Uploaded" />
              ) : (
                <img
            ...
            <div className="edit_body_info_text">
              ...
              <input
                className="edit_body_info_text_title"
                onChange={(e) => handleSongNameChange(e)}
                defaultValue={data && data.songname}
              />
                        ...
                        return (
                          ...
                          >
                            <div className="search_result_text">
                              <p className="search_result_text_name">
                                {data.genrename}
                              </p>
                            </div>
                          </div>
      ...
      <div className="gr">
        ...
        <button className="save_change" onClick={() => clearState()}>
          Huỷ
        </button>
      </div>
    </div>
